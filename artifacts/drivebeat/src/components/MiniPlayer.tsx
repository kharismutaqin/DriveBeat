...
export function MiniPlayer({ state, controls }: MiniPlayerProps) {
  ...
  const sleepLabel = sleepRemaining > 0 ? formatDuration(sleepRemaining) : null;
  ...
  return (
    ...
    >
      ...
      {showSleep && (
        <div className="absolute bottom-full left-4 right-4 mb-2 bg-[#111] border border-white/10 rounded-xl overflow-hidden shadow-2xl">
          <p className="text-white/30 text-[10px] uppercase tracking-widest px-4 pt-3 pb-1.5">Sleep Timer</p>
          <div className="flex flex-wrap gap-1.5 px-4 pb-3">
            {SLEEP_OPTIONS.map((opt) => (
              <button
                ...
                className={`px-3 py-1.5 rounded-lg text-xs font-medium transition-colors ${
                  sleepMinutes === opt.value && opt.value !== 0
                    ? "bg-white text-black"
                    : "bg-white/8 text-white/55 hover:bg-white/12"
                }`}
              >
      ...
      <div className="px-4 pt-3 pb-4">
        ...
        <div className="flex items-center justify-between">
          ...
          <button
            ...
            className={`flex items-center gap-1 px-2.5 py-1.5 rounded-lg text-xs transition-colors ${
              showSleep || sleepMinutes > 0
                ? "bg-white/12 text-white/80"
                : "text-white/30 hover:text-white/55 hover:bg-white/6"
            }`}
          >
            <Timer size={13} />
            <span>{sleepLabel ?? "Off"}</span>
          </button>
        </div>
      </div>
    </div>
